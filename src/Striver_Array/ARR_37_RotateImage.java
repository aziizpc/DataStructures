...
public class ARR_37_RotateImage {
	...
	public static int[][] getTranspose(int[][] arr) {
		for (int i = 1; i < arr.length; i++) { // We start from the element (1,0) and swap from the other part of the
												// diagonal. The loop goes till the last ROW and column j < i.
			for (int j = 0; j < i; j++) {
				int temp = arr[i][j];
				arr[i][j] = arr[j][i];
				arr[j][i] = temp;
