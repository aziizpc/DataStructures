...
public class ARR_41_SetMatrixZeroes {
	...
	public static int[][] setZeroes(int[][] matrix) {
		...
		for (int i = 0; i < n; i++) {			// All the rows
			for (int j = 0; j < m; j++) {		// All the columns
		...
		for (int i = 1; i < n; i++) {						  	// Loop FROM FIRST ROW (Not ZERO)
			for (int j = 1; j < m; j++) {					  	// Loop FROM FIRST COLUMN (NOT Zero)
		...
		if (matrix[0][0] == 0) {								// If the R0C0 value is 0
			for (int j = 0; j < m; j++)							// Set all the values in the R0 ROW to 0
				matrix[0][j] = 0;
		}

		if (col0 == 0) {										// If the COL0 value is 0
			for (int i = 0; i < n; i++)							// Set all the values in the C0 COLUMN to 0
				matrix[i][0] = 0;
		}
